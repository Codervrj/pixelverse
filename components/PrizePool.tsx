...
const prizes = [
  {
    ...
    offset: { desktop: { x: -380, y: -140, rotate: -10 }, mobile: { x: 0, y: -380, rotate: -5 } }
  },
  {
    rank: "01",
    ...
    sparkColor: "#FFD700", // Gold sparks
    offset: { desktop: { x: 0, y: -220, rotate: 0 }, mobile: { x: 0, y: -190, rotate: 0 } },
  ...
  },
  {
    ...
    sparkColor: "#CD7F32", // Bronze sparks
    offset: { desktop: { x: 380, y: -140, rotate: 10 }, mobile: { x: 0, y: -570, rotate: 5 } }
  }
];
...
const PrizePool: React.FC = () => {
  ...
  return (
    ...
    >
      ...
      {/* DYNAMIC MULTI-COLOR SPARKLE FIELD */}
      <AnimatePresence>
        {isOpen && (
          <motion.div 
            initial={{ opacity: 0 }}
            ...
            {[...Array(isMobile ? 30 : 85)].map((_, i) => (
              <motion.div 
                ...
                style={{ 
                  ...
                  left: `${(i * 17.3) % 100}%`, 
                  top: `${(i * 23.7) % 100}%`,
                  backgroundColor: activeColor, // DYNAMIC COLOR CHANGE
      ...
      <div className="max-w-7xl mx-auto px-6 relative z-10 flex flex-col items-center">
        ...
        <motion.div 
          ...
          animate={{ marginTop: isOpen ? (isMobile ? "580px" : "200px") : "0px" }}
          ...
          onMouseLeave={() => {
              if (!isMobile) {
                setIsOpen(false);
                setActiveColor("#FFD700"); // Reset to gold
              }
          }}
        >
          {prizes.map((prize, idx) => {
            ...
            return (
              <motion.div
                ...
                onMouseEnter={() => setActiveColor(prize.sparkColor)} // COLOR SWITCH TRIGGER
                whileHover={{ 
                  boxShadow: !isMobile ? `0px 0px 50px 15px ${prize.glowColor}, 8px 8px 0px 0px ${prize.shadowColor}` : undefined,
                  scale: !isMobile ? 1.05 : 1
                }}
                transition={{ type: "spring", stiffness: 150, damping: 20, delay: isOpen ? idx * 0.08 : 0 }}
                style={{ 
                  zIndex: isOpen ? (prize.rank === "01" ? 30 : 25) : 5, 
                  width: isMobile ? '250px' : '285px', 
                  height: '240px', 
                  left: isMobile ? '4px' : '17.5px', 
              ...
              >
                <div className="space-y-1 text-[#050008]">
                  <div className="flex justify-between items-center text-black/40 font-pixel text-[8px] font-bold uppercase tracking-widest">
                    <span>PV_DATA_NODE_{prize.rank}</span>
                ...
                {/* REDUCED FONT SIZE FOR 10000 */}
                <div className="flex-grow flex items-center justify-start overflow-hidden w-full">
                  <div className="text-[2.2rem] sm:text-[2.6rem] md:text-[2.8rem] font-archivo font-black text-[#050008] italic leading-none tracking-tighter whitespace-nowrap min-w-0 pr-2">
                    {prize.amount}
                  </div>
                </div>
          ...
          >
            <div className="absolute top-0 left-0 bg-[#050008] text-yellow-400 px-4 py-1.5 font-pixel text-[10px] font-bold uppercase">ROOT_DIR</div>
            <div className="space-y-1 md:space-y-2 text-[#050008]">
              <span className="font-pixel text-[8px] md:text-[10px] font-bold tracking-[0.3em] uppercase opacity-60 italic font-bold">{isOpen ? "SEQUENCING..." : "LOCKED"}</span>
              <div className="text-2xl md:text-3xl font-archivo font-black uppercase tracking-tighter leading-none">{isOpen ? "DIR_OPEN" : isMobile ? "TAP TO OPEN" : "DECRYPT"}</div>
            </div>
          </motion.div>
        </motion.div>
